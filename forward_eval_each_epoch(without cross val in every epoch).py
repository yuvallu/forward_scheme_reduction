...
def eval_accuracy(model, train_index, test_index, classifier='SVM'):
...
def save_mesures(model, train_index, test_index, mesures_df, i, e, s_time, classifier='SVM', steps_per_epoch=0):
    ...
    acc_e = eval_accuracy(model, train_index, test_index, classifier=classifier)
...
def train(model, loader, epochs, train_index, test_index, mesures_df, i, classifier='SVM', s_time=[0]):
    THRESHHOLD = 200
    model.to(device)
    model.train()
    ...
    for e in range(epochs):
        if time.time() - s_time[0] > THRESHHOLD:
            break
        save_mesures(model, train_index, test_index, mesures_df, i, e, s_time, classifier=classifier,
...
if __name__ == '__main__':
    ...
    for i, (train_index, test_index) in enumerate(split.split(rows, Y)):
        ...
        temp = [start]
        train(model, loader, args.epochs, train_index, test_index, mesures_df, i, classifier=args.classifier,
              s_time=temp)
        start = temp[0]
