...
def get_samples(db, depth, num_samples, sample_fct, yuval_change=''):
    ...
    #TODO run them:
        ...
        subset_full_schemes = [subset_full_schemes[idx] for idx in ordered_schemes_idxs][schemes_to_remove:]
    elif 'sorted_hepatitis_correct_highest_loss_after_10_epoch_' in yuval_change:  # highest loss first
        schemes_to_remove = int(yuval_change.split('sorted_hepatitis_correct_highest_loss_after_10_epoch_')[-1])
        ordered_schemes_idxs = [8, 5, 6, 7, 12, 13, 4, 9, 10, 11, 3, 2, 0, 1, 14]
        subset_full_schemes = [subset_full_schemes[idx] for idx in ordered_schemes_idxs][schemes_to_remove:]
    elif 'sorted_hepatitis_correct_lowest_loss_after_10_epoch_' in yuval_change:  # lowest loss first
        schemes_to_remove = int(yuval_change.split('sorted_hepatitis_correct_lowest_loss_after_10_epoch_')[-1])
        ...
        with open("temp_ordered_schemes.txt", 'r') as f:
            d = json.load(f)
