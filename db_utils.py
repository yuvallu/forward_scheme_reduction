...
class Database:
    ...
    def load_csv(path, index_col=0):
        ...
        # -# added because the order was different between linux and Windows machines
    ...
    def get_sigmod_graph(self, feature_size=300, add_row_classes=False, only_pred_relation=False,
                         add_relation_nodes=False):
    ...
    def scheme_tuple_map(self, rel_id, start_rows, depth, arrow_in=None, partition=None, backtrack=False):
        ...
        if depth > 0:
            # -# Changed in order to backtrack FK's
            if config["BACKTRACK"]["backtrack"] == 'True':
            ...
            for arrow_out in arrows_out:
                ...
                for meta_path, row_map in arrow_mpr_map.items():
                    tuple_map[f'{arrow_out}>{meta_path}'] = {
                        r: list(chain.from_iterable(row_map[s] for s in self.arrow_row_map[arrow_out][r])) for r in
                        start_rows}
